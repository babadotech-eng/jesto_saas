...
const ORIGEM_OPTIONS = [
  "Instagram", "Facebook", "TikTok", "Google", "Indicação de amigo", "YouTube", "Outro",
];
...
export default function Onboarding() {
  ...
  const [values, setValues] = useState<Record<FieldKey, string>>({
    nome_completo: "", nome_negocio: "", tipo_negocio: "", cidade_estado: "", whatsapp: "", origem: "",
  });
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F2] flex flex-col">
      ...
      <div className="flex items-center gap-2 p-6">
        <div className="bg-amber-400 text-zinc-900 p-2 rounded-lg">
          <BarChart3 size={20} />
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pb-20">
        <div className="w-full max-w-lg">
          ...
          <div className="flex gap-1.5 mb-10">
            {STEPS.map((_, i) => (
              <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= step ? "bg-amber-400" : "bg-zinc-200"}`} />
            ))}
          </div>
          ...
          <div className="mb-8">
            {current.required && (
              <span className="text-xs font-semibold text-amber-500 uppercase tracking-widest mb-2 block">
                Obrigatório
              </span>
            )}
          ...
          {current.type === "select" ? (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 mb-8">
              {ORIGEM_OPTIONS.map(opt => (
                ...
                >
                  {values.origem === opt && <Check size={12} className="inline mr-1 text-amber-500" />}
                  {opt}
                </button>
              ))}
            </div>
            ...
            <div className="mb-8">
              <Input
                ...
                autoFocus
                className="h-14 text-xl border-0 border-b-2 border-zinc-300 rounded-none bg-transparent px-0 focus-visible:ring-0 focus-visible:border-amber-400 transition-colors placeholder:text-zinc-300"
              />
            </div>
          )}
          ...
          <div className="flex items-center gap-4">
            <Button
              ...
              disabled={saving}
              size="lg"
              className="bg-amber-400 hover:bg-amber-500 text-zinc-900 font-bold px-8 rounded-xl h-12 shadow-lg shadow-amber-200"
            >
